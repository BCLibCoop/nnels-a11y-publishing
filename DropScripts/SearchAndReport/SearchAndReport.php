...
define ("DROPSCRIPT_OUTPUT_REPORT_PREFIX", ">!>:");
...
function postPostProcessFile($config, &$isModified, &$fileContents) {
    ...
    do { // non-loop
        try {
            ...
            if (! isset($searchPatterns)) {
                logNote("postProcessFile: no searchPatterns found in config");
                break;
            }

            foreach ($searchPatterns as $searchPattern) {
                $match = $searchPattern->match;
                $convertMatchFrom = $searchPattern->convertMatchFrom;
                $convertMatchTo = $searchPattern->convertMatchTo;
                try {
                    $matches = [];
                    $isFound = preg_match_all($match, $fileContents, $matches);
                    if ($isFound !== FALSE && $isFound > 0) {
                        foreach ($matches[0] as $match) {
                            $output = preg_replace($convertMatchFrom, $convertMatchTo, $match);
                            if (isset($output)) {
                                echo DROPSCRIPT_OUTPUT_REPORT_PREFIX . $config["droppedFileName"] . ": " . $output . "\n";
                            }
                        }
                    }
                }
