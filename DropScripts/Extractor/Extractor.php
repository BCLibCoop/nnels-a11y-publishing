...
function preProcessFile($config, &$isModified, &$fileContents) {
    ...
    logEntry("preProcessFile");

    // If you reverse the changes made here in postProcessFile, you should
    // not set isModified
       
    $extractions = &$config["extractions"];
    $filePathRelativeFromOEBPSPath = "";
    $filePath = $config["droppedFilePath"];
    $filePathSplit = preg_split("/[\\\\\\/]/", $filePath);
    $idx = count($filePathSplit) - 1;
    while ($idx >= 0 && $filePathSplit[$idx] != "OEBPS") {
        $filePathRelativeFromOEBPSPath = $filePathSplit[$idx] . ($filePathRelativeFromOEBPSPath ? "/" : "") . $filePathRelativeFromOEBPSPath;
        $idx--;
    }

    logNote("main: filePathRelativeFromOEBPSPath = '" . $filePathRelativeFromOEBPSPath . "'");
    ...
    foreach ($extractions as $extraction) {
        ...
        if (isset($extraction->outputFileNameExtension)) {
            $fileNameExtension = $extraction->outputFileNameExtension;
        }

        if (isset($config["epubFileName"])) {
            $outputFileName = $config["epubFileName"];
            $outputFolderPath = dirname($config["epubFilePath"]);
        }
        else {
            $outputFileName = $config["droppedFileName"];
            $outputFolderPath = dirname($config["droppedFilePath"]);
        }

        $oldFileNameExtension = pathinfo($outputFileName, PATHINFO_EXTENSION);
        $outputFileName = substr($outputFileName, 0, strlen($outputFileName) - strlen($oldFileNameExtension)) . $fileNameExtension;

        $outputFilePath = $outputFolderPath . DIRECTORY_SEPARATOR . $outputFileName;

        logNote("main: extraction #" . $extractionIdx . " output file path = " . $outputFilePath);
        
        $outputFile = null;

        $matches = [];
        if ($config["format"] == "JSON") {
            $patternFrom = $extraction->patternFrom;
            $patternTo = $extraction->patternTo;
        }
        else {
            $patternFrom = $extraction->patternFrom;
            if (substr($patternFrom,0,1) != "/" || substr($patternFrom,-1) != "/") {
                $patternFrom = "/" . preg_replace("/\\//", "\\/", $patternFrom) . "/";
            }
            $patternTo = $extraction->patternTo;
        }
        $patternTo = $extraction->patternTo;

        preg_match_all($patternFrom, $fileContents, $matches, PREG_SET_ORDER);

        foreach ($matches as $match) {

            $idx = count($match) - 1;

            $outputPattern = $extraction->patternTo;
            while ($idx >= 0) {
                $pattern = '/\\$' . $idx . '/';
                $outputPattern = preg_replace($pattern, $match[$idx], $outputPattern);
                $idx--;
            }

            $outputPattern = preg_replace('/\\$fileName/', $filePathRelativeFromOEBPSPath, $outputPattern);
            if (! $outputFile) {
                if (file_exists($outputFilePath)) {
                    logNote("main: appending to file " . $outputFilePath);
                    $outputFile = fopen($outputFilePath,"a");
                }
                else {
                    logNote("main: opening file " . $outputFilePath);
                    $outputFile = fopen($outputFilePath,"w");
                }
            }

            logTrace("main: extraction #" . $extractionIdx . " output pattern written: '" . $outputPattern . "'");

            fwrite($outputFile, $outputPattern . "\n");
        }

        logNote("main: extraction #" . $extractionIdx . " completed");
    ...
    logExit("preProcessFile");

}

function postProcessFile($config, &$isModified, &$fileContents) {

    logEntry("postProcessFile");

    // If this reverses the changes made in preProcessFile, you should
    // not set isModified
    // This is called before the locked headers have been restored

    logExit("postProcessFile");

}

function postPostProcessFile($config, &$isModified, &$fileContents) {

    logEntry("postPostProcessFile");

    // This is called after the locked headers have been restored

    logExit("postPostProcessFile");
}
